...
function fail_cdrs_pbx(xml,callerid_array,userinfo,destination_number)
    Logger.notice("[RINGGROUP] fail_cdrs_pbx ");
...
function did_pbx_info(xml,didinfo,userinfo,config,xml_did_rates,callerid_array)
    Logger.info("[RINGGROUP] did_pbx_info ");
        ...
        if(ringgroup_arr and ringgroup_arr ~= nil)then
            ...
            if(tonumber(Count) == 0)then
                ...
                if(call_direction == 'inbound')then
                    table.insert(xml, [[<action application="export" data="call_type=Ring Group"/>]]);
                end
                local sound_path = "/usr/share/freeswitch/sounds/pt/BR/karina/"
                audio_file = sound_path ..  "flux-badphone.wav";
...
--                table.insert(xml, [[<action application="info" data=""/>]]);
            ...
            for h=1,ringgroup_count do
                ...
                if (propmt_flag == 'false') then
                    group_confirm = "confirm=true,dialed_user="..params_ring[destination]..",";
                else
                    group_confirm = "confirm=true,dialed_user="..params_ring[destination]..",";
                end
            ...
            table.insert(xml, [[<action application="ring_ready"/>]]);
            ...
            table.insert(xml, [[<action application="set" data="variable_sip_to_host=$${domain_name}"/>]]);
            table.insert(xml, [[<action application="set" data="sip_destination_number=]]..destination_number..[["/>]]);
