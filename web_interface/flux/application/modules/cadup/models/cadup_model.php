...
// ##############################################################################
class cadup_model extends CI_Model
{
    ...
    function get_termination_cadup_list($flag, $start = 0, $limit = 0)
    {
        $this->db_model->build_search('termination_cadup_list_search');
    ...
    function get_termination_cadup($flag, $start = 0, $limit = 0, $export = true)
    {
        $this->db_model->build_search('termination_cadup_list_search');
    ...
    function get_origination_cadup($flag, $start = 0, $limit = 0, $export = true)
    {
        $this->db_model->build_search('origination_cadup_list_search');
    ...
    function get_origination_cadup_for_user($flag, $start = 0, $limit = 0, $export = true)
    {
        $this->db_model->build_search('origination_cadup_list_search');
    ...
    // ==============================================
    function get_origination_cadup_list($flag, $start = 0, $limit = 0)
    {
        $where = array();
        $this->db_model->build_search('origination_cadup_list_search');
        if ($this->session->userdata('logintype') == 1 || $this->session->userdata('logintype') == 5) {
...
//        $where['accountid'] = '0';
        $this->db_model->build_search('origination_cadup_list_search');
        if ($flag) {
            $query = $this->db_model->select("*", "view_cadup", $where, "id,nomePrestadora", "ASC", $limit, $start);
        } else {
            $query = $this->db_model->countQuery("*", "view_cadup", $where);
        }
        return $query;
    }
    ...
    function get_origination_cadup_list_for_user($flag, $start = 0, $limit = 0)
    {
        $this->db_model->build_search('user_cadup_list_search');
    ...
    function add_termination_cadup($add_array)
    {
    ...
    function edit_termination_cadup($data, $id)
    {
        unset($data["action"]);
        unset($data["id"]);
        /*
    ...
    function remove_termination_cadup($id)
    {
        $this->db->where("id", $id);
        $this->db->delete("outbound_routes");
        return true;
    ...
    function add_origination_cadup($add_array)
    {
        unset($add_array["action"]);
    ...
    function edit_origination_cadup($data, $id)
    {
        unset($data["action"]);
        unset($data["id"]);
        $data['last_modified_date'] = gmdate('Y-m-d H:i:s');
    ...
    function remove_origination_cadup($id)
    {
        $this->db->where("id", $id);
        $this->db->delete("routes");
        return true;
    ...
    function bulk_insert_termination_cadup($field_values, $count)
    {
    ...
    function bulk_insert_origination_cadup($field_values, $count)
    {
        $temp_arr = array();
        $keys = array_keys($field_values);
        $arr_key = array_keys($field_values[$keys[0]]);
    ...
    function termination_cadup_batch_update($update_array)
    {
        $this->db_model->build_search('termination_cadup_list_search');
        if ($this->session->userdata('logintype') == 1 || $this->session->userdata('logintype') == 5) {
            $account_data = $this->session->userdata("accountinfo");
    ...
    function origination_cadup_batch_update($update_array)
    {
        $this->db_model->build_search('origination_cadup_list_search');
        if ($this->session->userdata('logintype') == 1 || $this->session->userdata('logintype') == 5) {
            $account_data = $this->session->userdata("accountinfo");
    ...
    function getreseller_cadup_list($flag, $start = 0, $limit = 0, $export = false)
    {
        $account_data = $this->session->userdata("accountinfo");
        $price_list_id = $this->common->get_field_name('pricelist_id', 'accounts', $account_data['id']);
        $this->db_model->build_search('resellercadup_list_search');
        /*
         * $where = array (
         * "status" => "0",
    ...
    function get_cadupdeck_details($pattern)
    {
