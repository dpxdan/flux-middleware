...
// ##############################################################################
class Cadup extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->library('cadup_form');
        $this->load->library('flux/form', 'cadup_form');
        $this->load->library('flux/permission');
        $this->load->model('cadup_model');
    ...
    function termination_cadup_list()
    {
        ...
        $data['grid_fields'] = $this->cadup_form->build_termination_cadup_for_admin();
        $data["grid_buttons"] = $this->cadup_form->build_grid_buttons();
        $data['form_search'] = $this->form->build_serach_form($this->cadup_form->get_termination_cadup_search_form());
        $data['form_batch_update'] = $this->form->build_batchupdate_form($this->cadup_form->termination_cadup_batch_update_form());
        $this->load->view('view_termination_cadup_list', $data);
    }

    function termination_cadup_list_json()
    {
        $json_data = array();
        $count_all = $this->cadup_model->get_termination_cadup_list(false);
        $paging_data = $this->form->load_grid_config($count_all, $_GET['rp'], $_GET['page']);
        $json_data = $paging_data["json_paging"];

        $query = $this->cadup_model->get_termination_cadup_list(true, $paging_data["paging"]["start"], $paging_data["paging"]["page_no"]);
        $grid_fields = json_decode($this->cadup_form->build_termination_cadup_for_admin());
        $json_data['rows'] = $this->form->build_grid($query, $grid_fields);

        echo json_encode($json_data);
    }

    function termination_cadup_list_delete($flag = '')
    {
        $json_data = array();
    ...
    function termination_cadup_import()
    {
        $data['page_title'] = gettext('Import Termination Rates');
        $this->session->set_userdata('import_termination_cadup_csv', "");
        $this->session->set_userdata('import_termination_cadup_csv_error', "");
    ...
    function termination_cadup_preview_file()
    {
        $invalid_flag = false;
        ...
        $currency = $this->common->get_field_name('currency', 'currency', $currency_id);
        $data['currency'] = $currency;
        $config_termination_cadup_array = $this->config->item('Termination-rates-field');
        $this->db->where('id', $account_info['currency_id']);
        $this->db->select('currency');
        $currency_info = (array) $this->db->get('currency')->first_row();
        foreach ($config_termination_cadup_array as $key => $value) {
        ...
        if (! empty($_SERVER['CONTENT_LENGTH']) && empty($_FILES) && empty($_POST)) {
            ...
            $extension = explode(".", $_FILES['termination_cadup_import']['name']);
            if ((isset($extension[1])) && (! isset($extension[2]))) {
                if (isset($_FILES['termination_cadup_import']['name']) && $_FILES['termination_cadup_import']['name'] != "" && isset($_POST['trunk_id']) && $_POST['trunk_id'] != '') {
                    list ($txt, $ext) = explode(".", $_FILES['termination_cadup_import']['name']);
                    if ($ext == "csv" && $_FILES['termination_cadup_import']['size'] > 0) {
                        $error = $_FILES['termination_cadup_import']['error'];
                        $finfo = finfo_open(FILEINFO_MIME_TYPE);
                        $mime_type = finfo_file($finfo, $_FILES["termination_cadup_import"]["tmp_name"]);
                        ...
                        if (! in_array($mime_type, $acceptable_mime_types)) {
                            ...
                            if ($error == 0) {
                                $uploadedFile = $_FILES["termination_cadup_import"]["tmp_name"];
                                ...
                                if (! empty($csv_data)) {
                                    ...
                                    if (move_uploaded_file($uploadedFile, $full_path . $actual_file_name)) {
                                        ...
                                        $this->session->set_userdata('import_termination_cadup_csv', $actual_file_name);
                                    } else {
            ...
            if ($invalid_flag) {
                ...
                if (empty($_FILES['termination_cadup_import']['name'])) {
    ...
    function termination_cadup_rates_import($trunkID, $check_header = false)
    {
        $new_final_arr = array();
        $invalid_array = array();
        $row_count = 0;
        ...
        $full_path = $this->config->item('rates-file-path');
        $terminationrate_file_name = $this->session->userdata('import_termination_cadup_csv');
        ...
        if (! empty($new_final_arr)) {
            $this->cadup_model->bulk_insert_termination_rate($new_final_arr, $row_count);
        }
        ...
        if ($count > 0) {
            ...
            fclose($fp);
            $this->session->set_userdata('import_termination_cadup_csv_error', $session_id . ".csv");
            ...
            redirect(base_url() . "rates/termination_cadup_list/");
        }
    }

    function termination_cadup_error_download()
    {
        $this->load->helper('download');
        $error_data = $this->session->userdata('import_termination_cadup_csv_error');
        $full_path = $this->config->item('rates-file-path');
        ob_clean();
        $data = file_get_contents($full_path . $error_data);
        force_download(gettext("Termination_cadup_error").".csv", $data);
    }

    function origination_cadup_import()
    {
        $data['page_title'] = gettext('Import Origination Rates');
        $this->session->set_userdata('import_origination_cadup_csv', "");
        $error_data = $this->session->userdata('import_origination_cadup_csv_error');
        $full_path = $this->config->item('rates-file-path');
        $account_info = $this->session->userdata('accountinfo');
        $currency_id = $account_info['currency_id'];
        $currency = $this->common->get_field_name('currency', 'currency', $currency_id);
        ...
        if (file_exists($full_path . $error_data) && $error_data != "") {
            ...
            $this->session->set_userdata('import_origination_cadup_csv_error', "");
        }
        $this->load->view('view_import_origination_rate', $data);
    }

    function origination_cadup_preview_file()
    {
        $invalid_flag = false;
        $data = array();
        $data['page_title'] = gettext('Import Origination Rates');
        ...
        if (empty($_FILES) || ! isset($_FILES)) {
            redirect(base_url() . "rates/origination_cadup_list/");
        }
        $get_extension = strpos($_FILES['origination_cadup_import']['name'], '.');
        $config_orignination_cadup_array = $this->config->item('Origination-rates-field');
        $this->db->where('id', $account_info['currency_id']);
        $this->db->select('currency');
        $currency_info = (array) $this->db->get('currency')->first_row();
        foreach ($config_orignination_cadup_array as $key => $value) {
            $key = str_replace('CURRENCY', $currency_info['currency'], $key);
            $new_final_arr_key[$key] = $value;
        }
        ...
        $extension = explode(".", $_FILES['origination_cadup_import']['name']);
        if ((isset($extension[1])) && (! isset($extension[2]))) {
            if (isset($_FILES['origination_cadup_import']['name']) && $_FILES['origination_cadup_import']['name'] != "" && isset($_POST['pricelist_id']) && $_POST['pricelist_id'] != '') {
                list ($txt, $ext) = explode(".", $_FILES['origination_cadup_import']['name']);

                if ($ext == "csv" && $_FILES['origination_cadup_import']['size'] > 0) {
                    $finfo = finfo_open(FILEINFO_MIME_TYPE);
                    $mime_type = finfo_file($finfo, $_FILES["origination_cadup_import"]["tmp_name"]);
                    $acceptable_mime_types = array(
                        'application/csv',
                        'application/x-csv',
                        'text/csv',
                    ...
                    $error = $_FILES['origination_cadup_import']['error'];
                    if (! in_array($mime_type, $acceptable_mime_types)) {
                        $data['error'] = gettext("Invalid file format : Only CSV file allows to import records(Can't import empty file)");
                    } else {
                        if ($error == 0) {
                            $uploadedFile = $_FILES["origination_cadup_import"]["tmp_name"];
                            ...
                            if (! empty($csv_data)) {
                                ...
                                if (move_uploaded_file($uploadedFile, $full_path . $actual_file_name)) {
                                    ...
                                    $this->session->set_userdata('import_origination_cadup_csv', $actual_file_name);
                                } else {
                                    $data['error'] = gettext("File Uploading Fail Please Try Again");
                                }
                            }
        ...
        if ($invalid_flag) {
            ...
            if (empty($_FILES['origination_cadup_import']['name'])) {
    ...
    function origination_cadup_import_file($pricelistID, $trunkid, $check_header = false)
    {
        $new_final_arr = array();
        $invalid_array = array();
        ...
        $full_path = $this->config->item('rates-file-path');
        $originationrate_file_name = $this->session->userdata('import_origination_cadup_csv');
        ...
        if (! empty($new_final_arr)) {
            $this->cadup_model->bulk_insert_origination_rate($new_final_arr, $row_count);
        }
        ...
        if ($count > 0) {
            ...
            fclose($fp);
            $this->session->set_userdata('import_origination_cadup_csv_error', $session_id . ".csv");
            $data["error"] = $invalid_array;
    ...
    function origination_cadup_error_download()
    {
        $this->load->helper('download');
        $error_data = $this->session->userdata('import_origination_cadup_csv_error');
        $full_path = $this->config->item('rates-file-path');
        ob_clean();
        $data = file_get_contents($full_path . $error_data);
        force_download(gettext("Origination_cadup_error").".csv", $data);
    }

    function origination_cadup_add($type = "")
    {
        $data['username'] = $this->session->userdata('user_name');
        $data['flag'] = 'create';
        ...
        $data['form'] = $this->form->build_form($this->cadup_form->get_origination_cadup_form_fields(), '');
        $data['trunk_count'] = Common_model::$global_config['system_config']['trunk_count'];
        $this->load->view('view_origination_cadup_add_edit', $data);
    }

    function origination_cadup_edit($edit_id = '')
    {
        $this->permission->check_web_record_permission($edit_id, 'routes', 'rates/origination_cadup_list/');
        ...
        if ($account->num_rows() > 0) {
            ...
            $data['trunk_count'] = Common_model::$global_config['system_config']['trunk_count'];
            $data['form'] = $this->form->build_form($this->cadup_form->get_origination_cadup_form_fields($edit_id, $reseller_id), $edit_data);
    ...
    function origination_cadup_save()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $data['trunk_count'] = Common_model::$global_config['system_config']['trunk_count'];
            $data['form'] = $this->form->build_form($this->cadup_form->get_origination_cadup_form_fields(), $add_array);
            if ($add_array['id'] != '') {
                $data['page_title'] = gettext('Edit Origination Rate');
                if ($this->form_validation->run() == FALSE) {
                    ...
                    $this->cadup_model->edit_origination_rate($add_array, $add_array['id']);
		    ...
		    $add_array['trunk_id'] = (isset($add_array['trunk_id']) && $add_array['trunk_id'] > 0)?$add_array['trunk_id']:0;
                    $last_id = $this->cadup_model->add_origination_rate($add_array);
    ...
    function origination_cadup_list_search()
    {
        ...
        if ($this->input->post('advance_search', TRUE) == 1) {
            ...
            $this->session->set_userdata('origination_cadup_list_search', $action);
        }
        if (@$ajax_search != 1) {
            redirect(base_url() . 'rates/origination_cadup_list/');
        }
    ...
    function origination_cadup_list_clearsearchfilter()
    {
        $this->session->set_userdata('advance_search', 0);
        $this->session->set_userdata('account_search', "");
    }

    function origination_cadup_list()
    {
        $data['username'] = $this->session->userdata('user_name');
        ...
        $data['delete_batch_flag'] = true;
        $this->session->set_userdata('advance_search', 0);
        $data['grid_fields'] = $this->cadup_form->build_origination_cadup_list_for_admin();
        $data["grid_buttons"] = $this->cadup_form->build_grid_buttons_origination_rate();
        $data['form_search'] = $this->form->build_serach_form($this->cadup_form->get_origination_cadup_search_form());
        $data['form_batch_update'] = $this->form->build_batchupdate_form($this->cadup_form->origination_cadup_batch_update_form());
        $this->load->view('view_origination_cadup_list', $data);
    }

    function origination_cadup_list_json()
    {
        $json_data = array();
        $count_all = $this->cadup_model->get_origination_cadup_list(false);
        $paging_data = $this->form->load_grid_config($count_all, $_GET['rp'], $_GET['page']);
        $json_data = $paging_data["json_paging"];
        $query = $this->cadup_model->get_origination_cadup_list(true, $paging_data["paging"]["start"], $paging_data["paging"]["page_no"]);

        $grid_fields = json_decode($this->cadup_form->build_origination_cadup_list_for_admin());
        $json_data['rows'] = $this->form->build_grid($query, $grid_fields);
        $result = $query->result_array();
    ...
    function origination_cadup_list_delete($flag = '')
    {
        $json_data = array();
        $this->session->set_userdata('advance_batch_data_delete', 1);
        $count_all = $this->cadup_model->get_origination_cadup_list(false);
    ...
    function termination_cadup_add($type = "")
    {
        $data['username'] = $this->session->userdata('user_name');
        $data['flag'] = 'create';
        $data['page_title'] = gettext('Create Termination Rate');
        $data['form'] = $this->form->build_form($this->cadup_form->get_termination_cadup_form_fields(), '');
        $this->load->view('view_termination_cadup_add_edit', $data);
    }

    function termination_cadup_edit($edit_id = '')
    {
        ...
        $edit_data['pattern'] = preg_replace('/[^\d+0-9]/', '', $edit_data['pattern']);
        $data['form'] = $this->form->build_form($this->cadup_form->get_termination_cadup_form_fields(), $edit_data);
        $this->load->view('view_termination_cadup_add_edit', $data);
    }

    function termination_cadup_save()
    {
        $add_array = $this->input->post();
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $data['form'] = $this->form->build_form($this->cadup_form->get_termination_cadup_form_fields(), $add_array);
            if ($add_array['id'] != '') {
                $data['page_title'] = gettext('Edit Termination Rate');
                if ($this->form_validation->run() == FALSE) {
                    $data['validation_errors'] = validation_errors();
                    ...
                    $add_array['connectcost'] = isset($add_array['connectcost']) && ($add_array['connectcost'] != "") ? $this->common_model->add_calculate_currency($add_array['connectcost'], '', '', false, false) : 0;
                    $add_array['cost'] = isset($add_array['cost']) && ($add_array['cost'] != "") ? $this->common_model->add_calculate_currency($add_array['cost'], '', '', false, false) : 0;
                    $this->cadup_model->edit_termination_rate($add_array, $add_array['id']);
                    echo json_encode(array(
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    $add_array['connectcost'] = isset($add_array['connectcost']) && ($add_array['connectcost'] != "") ? $this->common_model->add_calculate_currency($add_array['connectcost'], '', '', false, false) : 0;
                    $add_array['cost'] = isset($add_array['cost']) && ($add_array['cost'] != "") ? $this->common_model->add_calculate_currency($add_array['cost'], '', '', false, false) : 0;
                    $this->cadup_model->add_termination_rate($add_array);
                    echo json_encode(array(
    ...
    function termination_cadup_list_search()
    {
        $ajax_search = $this->input->post('ajax_search', 0);
        ...
        if ($this->input->post('advance_search', TRUE) == 1) {
            ...
            unset($action['action']);
            unset($action['advance_search']);
            $this->session->set_userdata('termination_cadup_list_search', $action);
        }
        if (@$ajax_search != 1) {
            redirect(base_url() . 'rates/termination_cadup_list/');
        }
    ...
    function termination_cadup_list_clearsearchfilter()
    {
        $this->session->set_userdata('advance_search', 0);
        $this->session->set_userdata('account_search', "");
    }
    ...
    function customer_block_pattern_list($accountid, $accounttype)
    {
        ...
        $grid_fields = json_decode($this->cadup_form->build_pattern_list_for_customer($accountid, $accounttype));
        $json_data['rows'] = $this->form->build_grid($pattern_data, $grid_fields);
        echo json_encode($json_data);
    }

    function termination_cadup_delete_multiple()
    {
    ...
    function origination_cadup_delete_multiple()
    {
        $ids = $this->input->post("selected_ids", true);
        if (isset($ids) && $ids != "") {
            $where = "id IN ($ids)";
    ...
    function user_origination_cadup_list_json()
    {
        $json_data = array();
        $account_data = $this->session->userdata("accountinfo");
        ...
        $count_all = $this->cadup_model->get_origination_cadup_list_for_user(false);
        $paging_data = $this->form->load_grid_config($count_all, $_GET['rp'], $_GET['page']);
        $json_data = $paging_data["json_paging"];

        $query = $this->cadup_model->get_origination_cadup_list_for_user(true, $paging_data["paging"]["start"], $paging_data["paging"]["page_no"]);
        $grid_fields = json_decode($this->cadup_form->build_origination_cadup_list_for_user());
    ...
    function user_origination_cadup_list_search()
    {
        $ajax_search = $this->input->post('ajax_search', 0);
        ...
        if (@$ajax_search != 1) {
            redirect(base_url() . 'user/user_cadup_list/');
        }
    }

    function user_origination_cadup_list_clearsearchfilter()
    {
        $this->session->set_userdata('advance_search', 0);
        $this->session->set_userdata('account_search', "");
    }
    ...
    function termination_cadup_batch_update()
    {
        ...
        $result = $this->cadup_model->termination_cadup_batch_update($batch_update_arr);
        echo json_encode(array(
    ...
    function origination_cadup_batch_update()
    {
        $batch_update_arr = $this->input->post();
        ...
        $result = $this->cadup_model->origination_cadup_batch_update($batch_update_arr);
        echo json_encode(array(
    ...
    function termination_cadup_export_cdr_xls()
    {
        $account_info = $accountinfo = $this->session->userdata('accountinfo');
        $currency_id = $account_info['currency_id'];
        $currency = $this->common->get_field_name('currency', 'currency', $currency_id);
        $query = $this->cadup_model->get_termination_rate(true, '', '', false);
    ...
    function termination_cadup_export_cdr_pdf()
    {
        $query = $this->cadup_model->get_termination_rate(true, '', '', false);
        $outbound_array = array();
    ...
    function origination_cadup_export_cdr_xls()
    {
        $account_info = $accountinfo = $this->session->userdata('accountinfo');
        $currency_id = $account_info['currency_id'];
        $reseller_id = $accountinfo['reseller_id'] > 0 ? $accountinfo['reseller_id'] : 0;
        $currency = $this->common->get_field_name('currency', 'currency', $currency_id);
        $query = $this->cadup_model->get_origination_rate(true, '', '', false);
    ...
    function origination_cadup_export_cdr_pdf()
    {
        $query = $this->cadup_model->get_origination_rate(true, '', '', false);
    ...
    function user_origination_cadup_cdr_pdf()
    {
        $query = $this->cadup_model->get_origination_cadup_for_user(true, '', '', false);
        $inbound_array = array();
        $this->load->library('fpdf');
        $this->load->library('pdf');
        $this->fpdf = new PDF('P', 'pt');
    ...
    function resellerscadup_list()
    {
        ...
        $data['search_flag'] = true;
        $this->session->set_userdata('advance_search', 0);
        $data['grid_fields'] = $this->cadup_form->build_cadup_list_for_reseller();
        $data["grid_buttons"] = $this->cadup_form->build_grid_buttons_rates();
        $data['form_search'] = $this->form->build_serach_form($this->cadup_form->get_reseller_origination_cadup_search_form());
        $this->load->view('view_resellerscadup_list', $data);
    }

    function resellerscadup_list_json()
    {
        $json_data = array();
        $account_data = $this->session->userdata("accountinfo");
        $markup = $this->common->get_field_name('markup', 'pricelists', array(
        ...
        $count_all = $this->cadup_model->getreseller_cadup_list(false);
        $paging_data = $this->form->load_grid_config($count_all, $_GET['rp'], $_GET['page']);
        $json_data = $paging_data["json_paging"];
        $query = $this->cadup_model->getreseller_cadup_list(true, $paging_data["paging"]["start"], $paging_data["paging"]["page_no"]);
        $grid_fields = json_decode($this->cadup_form->build_cadup_list_for_reseller());
        foreach ($query->result_array() as $key => $value) {
            $cost = 0;
    ...
    function resellerscadup_list_search()
    {
        $ajax_search = $this->input->post('ajax_search', 0);
        ...
        if ($this->input->post('advance_search', TRUE) == 1) {
            ...
            unset($action['action']);
            unset($action['advance_search']);
            $this->session->set_userdata('resellercadup_list_search', $action);
        }
        if (@$ajax_search != 1) {
            redirect(base_url() . 'rates/resellerscadup_list/');
        }
    }

    function resellerscadup_list_clearsearchfilter()
    {
        $this->session->set_userdata('advance_search', 0);
        $this->session->set_userdata('resellercadup_list_search', "");
    }
    ...
    function resellersrates_xls()
    {
        ...
        $currency_id = $account_info['currency_id'];
        $currency = $this->common->get_field_name('currency', 'currency', $currency_id);
        $query = $this->cadup_model->getreseller_cadup_list(true, '0', '0', '1');
    ...
    function termination_cadup_list_batch_delete()
    {
        $ajax_search = $this->input->post('ajax_search', 0);
        if ($this->input->post('advance_search', TRUE) == 1) {
    ...
    function origination_cadup_list_batch_delete()
    {
        $ajax_search = $this->input->post('ajax_search', 0);
        if ($this->input->post('advance_search', TRUE) == 1) {
            $this->session->set_userdata('advance_batch_delete', $this->input->post('advance_search'));
    ...
    function termination_cadup_import_mapper()
    {
        $data['page_title'] = gettext('Import Termination Rates Using Field Mapper');
        $this->session->set_userdata('import_termination_cadup_mapper_csv', "");
        $this->session->set_userdata('import_termination_cadup_mapper_csv_error', "");
        $account_info = $this->session->userdata('accountinfo');
        $currency_id = $account_info['currency_id'];
        $currency = $this->common->get_field_name('currency', 'currency', $currency_id);
        $data['currency'] = $currency;
        $this->load->view('view_import_termination_cadup_mapper', $data);
    }
    ...
    function termination_cadup_mapper_preview_file()
    {
        $invalid_flag = false;
        $check_header = $this->input->post('check_header', true);
        $data['page_title'] = gettext('Import Termination Rates');
        ...
        if (! empty($_SERVER['CONTENT_LENGTH']) && empty($_FILES) && empty($_POST)) {
            ...
            $extension = explode(".", isset($_FILES['termination_cadup_import_mapper']['name']) ? $_FILES['termination_cadup_import_mapper']['name'] : '');
            if ((isset($extension[1])) && (! isset($extension[2]))) {
                if (isset($_FILES['termination_cadup_import_mapper']['name']) && $_FILES['termination_cadup_import_mapper']['name'] != "" && isset($_POST['trunk_id']) && $_POST['trunk_id'] != '') {
                    list ($txt, $ext) = explode(".", $_FILES['termination_cadup_import_mapper']['name']);
                    if ($ext == "csv" && $_FILES['termination_cadup_import_mapper']['size'] > 0) {
                        $finfo = finfo_open(FILEINFO_MIME_TYPE);
                        $mime_type = finfo_file($finfo, $_FILES["termination_cadup_import_mapper"]["tmp_name"]);
                        $acceptable_mime_types = array(
                            'application/csv',
                            'application/x-csv',
                            'text/csv',
                        ...
                        $error = $_FILES['termination_cadup_import_mapper']['error'];
                        if (! in_array($mime_type, $acceptable_mime_types)) {
                            $data['error'] = gettext("Invalid file format : Only CSV file allows to import records(Can't import empty file)");
                        } else {
                            if ($error == 0) {
                                $uploadedFile = $_FILES["termination_cadup_import_mapper"]["tmp_name"];
                                ...
                                if (! empty($csv_data)) {
                                    ...
                                    if (move_uploaded_file($uploadedFile, $full_path . $actual_file_name)) {
                                        ...
                                        $this->session->set_userdata('import_termination_cadup_mapper_csv', $actual_file_name);
                                    } else {
                                        $data['error'] = gettext("File Uploading Fail Please Try Again");
                                    }
                                }
        ...
        if ($invalid_flag) {
            ...
            if (empty($_FILES['termination_cadup_import_mapper']['name'])) {
                $str .= '<div class="col-12">'.gettext("Please Select File.").'</div>';
            }
    ...
    function termination_cadup_rates_mapper_import()
    {
        $row_count = 0;
        ...
        $full_path = $this->config->item('rates-file-path');
        $terminationrate_file_name = $this->session->userdata('import_termination_cadup_mapper_csv');
        $csv_tmp_data = $this->csvreader->parse_file($full_path . $terminationrate_file_name, $new_final_arr_key, $check_header);
        ...
        if (! empty($new_final_arr)) {
            $result = $this->cadup_model->bulk_insert_termination_rate($new_final_arr, $row_count);
        }
        ...
        if ($count > 0) {
            ...
            fclose($fp);
            $this->session->set_userdata('import_termination_cadup_mapper_csv_error', $session_id . ".csv");
            $data["error"] = $invalid_array;
            $data['trunkid'] = $trunkID;
            $data['impoted_count'] = count($new_final_arr);
            $data['failure_count'] = count($invalid_array) - 1;
    ...
    function termination_cadup_mapper_error_download()
    {
        $this->load->helper('download');
        $error_data = $this->session->userdata('import_termination_cadup_mapper_csv_error');
        $full_path = $this->config->item('rates-file-path');
        $data = file_get_contents($full_path . $error_data);
        force_download(gettext("Termination_cadup_mapper_error").".csv", $data);
    }
