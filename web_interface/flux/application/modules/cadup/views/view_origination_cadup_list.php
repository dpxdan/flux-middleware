...
function search_btn(){
    document.getElementById('origination_cadup_batch_dlt').style.display = 'block';
}
function check_btn(){
	 	$.ajax({
			type: "POST",
			url: "<?= base_url()?>/cadup/origination_cadup_list_delete/",
			data:'',
			success:function(alt) {
			 if(alt > 0){
		  	   post_request_for_batch_delete("origination_cadup_grid",alt,"origination_cadup_list_search");
	  	           $('#origination_cadup_grid').flexOptions({newp:1}).flexReload();
			  }else{
    ...
    $(document).ready(function() {
        build_grid("origination_cadup_grid","",<? echo $grid_fields; ?>,<? echo $grid_buttons; ?>);
        $('.checkall').click(function () {
            $('.chkRefNos').prop('checked', $(this).prop('checked')); //if you want to select/deselect checkboxes use this
        });
        $("#origination_cadup_list_search_btn").click(function(){
            post_request_for_search("origination_cadup_grid","","origination_cadup_list_search");
        });        
        $("#id_reset").click(function(){
    document.getElementById('origination_cadup_batch_dlt').style.display = 'none';
            clear_search_request("origination_cadup_grid","");
            $("#pricelist_id_search_drp").html("<option value='' selected='selected'><?php echo gettext("--Select--")?></option>");
        });

         $("#batch_update").click(function(){
            submit_form("origination_cadup_batch_update");
        })
	...
	<div class="w-section inverse p-0">
		<div class="card col-md-12 pb-4">
			<form method="POST" enctype="multipart/form-data" id="ListForm">
				<table id="origination_cadup_grid" align="left"
