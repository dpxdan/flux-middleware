...
//            $counters = $this->db->get_where('packages_view', array('counters_package_id' => $package->id))->result_object();
            ...
            if (empty($counters)) {
                ...
                $invoicePackage->usedMinutes = gmdate('i:s', $package->counters_used_minutes);
            }
    ...
    private function calculateTaxes($invoice,$filter) {
        $summaryTaxes = new stdClass();
        ...
        $this->db->select('taxes_rate,taxes_description,accountid');
        $this->db->from('taxes');
        $this->db->join('taxes_to_accounts', 'taxes.id = taxes_to_accounts.taxes_id');
        $this->db->where(array(
            'taxes_to_accounts.accountid' => $filter->accountid,
            'taxes.status' => '0'
