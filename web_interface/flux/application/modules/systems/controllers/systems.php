...
class Systems extends MX_Controller
{
    ...
    function configuration($group_title = '')
    {
        ...
        $this->flux_log->write_log ( 'system_model', json_encode($data['details']) );
        ...
        if (! empty($add_array)) {
            ...
            $this->flux_log->write_log ( 'get_system_sidemenue', json_encode($data['menu']) );
    ...
    function database_backup()
    {
        ...
        $this->flux_log->write_log('database_backup', json_encode($this->db->database));
...
//                $db_name = $this->db->database;
                ...
                $run_backup = "/usr/bin/mysqldump -all --no-create-db --routines --triggers --events --databases " . $db_name . " -u'" . $db_username . "' -p'" . $db_password . "' > '$backup_file'";
                $this->flux_log->write_log ( 'database_backup_save', json_encode($add_array['db_name']) );
                $error_zip = 0;
    ...
    function database_restore_one($id = '')
    {
        ...
        if ($result->num_rows() > 0) {
            //$db_name = $this->db->database;
            $db_name = $result_array[0]['db_name'];
            $db_username = $this->db->username;
            $db_password = $this->db->password;
            $db_hostname = $this->db->hostname;
    ...
    function database_import_file()
    {
        if ($_POST['fname'] !='' && $_FILES['userfile']['name'] != '' && $_FILES['userfile']['type'] == "application/gzip") {
            ...
            if (file_exists($filename)) {
                ...
                if ($db_file[1] == 'csv' || $db_file[1] == 'tar' || $db_file[1] == 'sql') {
                    ...
                    $query = $this->system_model->import_database($filename_text, $_FILES['userfile']['name'], $filename_db);
        ...
        elseif ($_POST['fname'] !='' && $_FILES['userfile']['name'] != '' && $_FILES['userfile']['type'] == "application/x-gzip") {
                    $filename_text = $_POST['fname'];
                    $filename_db = $_POST['dname'];
                    $upload_greeting_file = $_FILES['userfile']['name'];
                    $filename = DATABASE_DIRECTORY . $upload_greeting_file;
    ...
    {
        $data['username'] = $this->session->userdata('user_name');
        $data['page_title'] = gettext('API');
        $data['search_flag'] = true;
        $data['cur_menu_no'] = 4;
        $this->session->set_userdata('advance_search', 0);
        $this->session->set_userdata('api_list_search', '');
        $data['grid_fields'] = $this->system_form->build_api_list_for_admin();
        if ($this->session->userdata('logintype') == 2) {
            $data["grid_buttons"] = $this->system_form->build_admin_api_grid_buttons();
        } else {
            $data["grid_buttons"] = json_encode(array());
        }
        $data['form_search'] = $this->form->build_serach_form($this->system_form->get_search_api_form());
        $this->load->view('view_api_list', $data);
    }

    function api_list_json()
    {
        $json_data = array();
        $count_all = $this->system_model->getapi_list(false);
        $paging_data = $this->form->load_grid_config($count_all, $_GET['rp'], $_GET['page']);
        $json_data = $paging_data["json_paging"];
        $query = $this->system_model->getapi_list(true, $paging_data["paging"]["start"], $paging_data["paging"]["page_no"]);
        $grid_fields = json_decode($this->system_form->build_api_list_for_admin());
        $json_data['rows'] = $this->form->build_grid($query, $grid_fields);

        echo json_encode($json_data);
    }

    function api_list_search()
    {
        $ajax_search = $this->input->post('ajax_search', 0);
        ...
        if ($this->input->post('advance_search', TRUE) == 1) {
            ...
            unset($action['action']);
            unset($action['advance_search']);
            $this->session->set_userdata('api_list_search', $action);
        }
        if (@$ajax_search != 1) {
            redirect(base_url() . 'systems/api_list/');
        }
    }

    function api_list_clearsearchfilter()
    {
        $this->session->set_userdata('advance_search', 0);
        $this->session->set_userdata('api_list_search', "");
    }
    ...
    function api_add()
    {
        $data['username'] = $this->session->userdata('user_name');
        $data['flag'] = 'create';
        $data['page_title'] = gettext('Add API');
        $data['form'] = $this->form->build_form($this->system_form->get_api_form_fields(), '');
        $this->load->view('view_api_add_edit', $data);
    }

    function api_list_edit($edit_id = '')
    {
        $data['page_title'] = gettext('Edit API');
        $where = array(
            'id' => $edit_id
        );
        ...
        $data['form'] = $this->form->build_form($this->system_form->get_api_form_fields($edit_id), $edit_data);
        $this->load->view('view_api_add_edit', $data);
    }
    ...
    {
        $add_array = $this->input->post();
        $data['form'] = $this->form->build_form($this->system_form->get_api_form_fields($add_array['id']), $add_array);
        if ($add_array['id'] != '') {
            $data['page_title'] = gettext('Edit API');
            if ($this->form_validation->run() == FALSE) {
                $data['validation_errors'] = validation_errors();
                ...
                $this->system_model->edit_api($add_array, $add_array['id']);
                echo json_encode(array(
                    "SUCCESS" => $add_array["endpoint_name"].' '.gettext('Endpoint Updated successfully!')
                ));
                exit();
            }
        } else {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $response = $this->system_model->add_api($add_array);
                echo json_encode(array(
                    "SUCCESS" => $add_array["endpoint_name"].' '.gettext('Endpoint Added successfully!')
                ));
                exit();
            }
        }
    ...
    {
        $this->system_model->remove_api($id);
        $api = $this->common->get_field_name('*', 'api_endpoints', $id);
        $this->session->set_flashdata('flux_notification', $api.' '.gettext('Endpoint removed successfully!'));

        redirect(base_url() . 'systems/api_list/');
    }

    function api_delete_multiple()
    {
        $ids = $this->input->post("selected_ids", true);
        $where = "id IN ($ids)";
        $this->db->where($where);
