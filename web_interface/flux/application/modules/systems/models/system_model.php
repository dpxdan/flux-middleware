...
class System_model extends CI_Model
{
    ...
    function getapi_list($flag, $start = 0, $limit = 0)
    {
        $this->db_model->build_search('api_list_search');
        $where = array();
        if ($flag) {
            if (isset($_GET['sortname']) && $_GET['sortname'] == 'id') {
                $_GET['sortname'] = "endpoint_name";
                $_GET['sortorder'] = 'ASC';
            }
            $query = $this->db_model->select("*", "api_endpoints", $where, "endpoint_name", "ASC", $limit, $start);
        } else {
    ...
    function import_database($filename, $target_path, $dbname)
    {
