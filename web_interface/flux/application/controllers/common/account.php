...
class Account extends API_Controller {
	...
	protected function _authorize_account($account_info, $balance = true, $credit_limit = true) {
		...
		$this->api_log->write_log ( 'account_info_reformat', json_encode($account_info) );
	...
	protected function _account_info_reformat($account_info, $balance = true, $credit_limit = true ) {
		...
		if (@$account_info ['balance'] != '' && @$account_info ['credit_limit'] != '' && @$account_info ['posttoexternal'] != '') {
			...
			$account_info ['display_balance'] = $this->common_model->calculate_currency ( $account_info ['display_balance'], '', $account_info ['currency_id'], TRUE, TRUE );
		}
		if (isset ( $account_info ['credit_limit'] )) {
			$account_info ['credit_limit'] = $this->common_model->calculate_currency ( $account_info ['credit_limit'], '', $account_info ['currency_id'], $balance, $credit_limit );
		}
		if (isset ( $account_info ['balance'] )) {
			$account_info ['balance'] = $this->common_model->calculate_currency ( $account_info ['balance'], '', $account_info ['currency_id'], $balance, $credit_limit );
		}
