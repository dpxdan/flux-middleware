...
//					$value['city'] = $value['city'];
	...
	function _customer_create() {
			$postdata = $this->postdata;
			// Se a conta for do tipo 1, define diretamente o reseller_id
			if ($this->accountinfo['type'] == 1) {
			    $postdata['reseller_id'] = $this->accountinfo['id'];
			} 
			else {
			    // Garante que reseller_id exista e seja válido
			    $postdata['reseller_id'] = $postdata['reseller_id'] ?? 0;
			
			    if (empty($postdata['reseller_id'])) {
			        $postdata['reseller_id'] = 0;
			    } else {
			        if ($postdata['reseller_id'] > 0) {
			            $postdata['reseller_id'] = $this->common->get_field_name(
			                'id',
			                'accounts',
			                ['id' => $postdata['reseller_id'], 'type' => 1, 'deleted' => 0]
			            );
			        }
		...
		$postdata['pricelist_id'] = $postdata['pricelist_id'] ?? 1;
		if($postdata['pricelist_id'] == '' || !isset($postdata['pricelist_id'])){
				...
				$postdata['pricelist_id'] = $this->common->get_field_name('id','pricelists',array('id'=>$postdata['pricelist_id'], 'reseller_id' => $postdata['reseller_id']));
			}
		...
		if(empty($postdata['pricelist_id'] ) ){
			...
			$config = Common_model::$global_config['system_config']['cardlength'];
//			$config = common_model::$global_config['system_config']['card_length'];
			if($config != ''){
				$uname = $this->common->find_uniq_rendno_customer(Common_model::$global_config['system_config']['cardlength'], 'number ', 'accounts');	
			}
			else{
			...
			$sweep_id = isset($postdata['sweep_id']) ? $postdata['sweep_id'] : '';
			
			if ($sweep_id == '' || $sweep_id >= '3' || $sweep_id == '1') {
			    $sweep_id = 2;
			}

			if($postdata['status'] == 'S'){
					$status = 0;
			 }
			 elseif($postdata['status'] == 'N'){
			 		$status = 1;
			  }
			else{
					$status =  0;
			...
			if (!isset($postdata['invoice_day']) || !($postdata['invoice_day'] == -1 || $postdata['invoice_day'] <= 30)) {
		...
		if (!isset($postdata['posttoexternal']) || !($postdata['posttoexternal'] == 0 || $postdata['posttoexternal'] == 1)) {
		    $postdata['posttoexternal'] = '0';
		}


		if ((!isset($postdata['credit_limit']) || $postdata['credit_limit'] == '') && isset($postdata['posttoexternal']) && $postdata['posttoexternal'] == 1) {
		    $postdata['credit_limit'] = 10000.00;
		}
				
		
		if (isset($postdata['credit_limit']) && !is_numeric($postdata['credit_limit']) && $postdata['credit_limit'] != '') {
		    $this->response(array(
		        'status' => false,
		...
		if (isset($postdata['tax_number']) && !is_numeric($postdata['tax_number']) && $postdata['tax_number'] != '') {
		    $this->response(array(
		        'status' => false,
		        'error'  => $this->lang->line('valid_credit_limit')
		    ), 400);
		...
		$postdata['notification_email'] = (isset($postdata['action']) && $postdata['action'] == 'provider_create' && isset($postdata['email'])) 
		    ? $postdata['email'] 
	...
	function _customer_create_new() {
	    $postdata = $this->postdata;
	    $configget = $this->common_model::$global_config ["system_config"]["cardlength"];
	    $this->api_log->write_log ( 'get_system_config : ', json_encode($configget) );
	    // Se a conta for do tipo 1, define diretamente o reseller_id
	    if ($this->accountinfo['type'] == 1) {
	        $postdata['reseller_id'] = $this->accountinfo['id'];
	    } else {
	        $postdata['reseller_id'] = isset($postdata['reseller_id']) ? $postdata['reseller_id'] : 0;
	        ...
	        if (empty($postdata['reseller_id'])) {
	            ...
	            if ($postdata['reseller_id'] > 0) {
	                $postdata['reseller_id'] = $this->common->get_field_name(
	                    'id', 'accounts',
	                    array('id' => $postdata['reseller_id'], 'type' => 1, 'deleted' => 0)
	                );
	            }
	    ...
	    // Definição do pricelist_id
	    $postdata['pricelist_id'] = isset($postdata['pricelist_id']) ? $postdata['pricelist_id'] : 1;
	
	    if (empty($postdata['pricelist_id'])) {
	        $postdata['pricelist_id'] = 1;
	    } else {
	        $postdata['pricelist_id'] = $this->common->get_field_name(
	            'id', 'pricelists',
	            array('id' => $postdata['pricelist_id'], 'reseller_id' => $postdata['reseller_id'])
	        );
	...
	//			$config = common_model::$global_config['system_config']['card_length'];
	    ...
	    $encoded_password = $this->common->encode($postdata['password']);
	    $sweep_id = (empty($postdata['sweep_id']) || $postdata['sweep_id'] >= 3 || $postdata['sweep_id'] == 1) ? 2 : $postdata['sweep_id'];
	    $status = ($postdata['status'] == 'N') ? 1 : 0;
	
	    $insert_array = array(
	        'number' => $uname,
	        'password' => $encoded_password,
	        'email' => $postdata['email'],
	        'pin' => isset($pin_number) ? $pin_number : '',
	        'status' => $status,
	        'reseller_id' => $postdata['reseller_id'],
	        'first_name' => $postdata['first_name'],
	        'company_name' => isset($postdata['company_name']) ? $postdata['company_name'] : '',
	        'last_name' => isset($postdata['last_name']) ? $postdata['last_name'] : '',
	        'telephone_1' => isset($postdata['telephone_1']) ? $postdata['telephone_1'] : '',
	        'pricelist_id' => isset($postdata['pricelist_id']) ? $postdata['pricelist_id'] : 1,
	        'creation' => gmdate('Y-m-d H:i:s')
	    );
