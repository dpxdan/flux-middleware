...
class common {
	...
	function get_cron_status($select = "", $table = "", $status) {
			if(isset($status['reseller_status'])){
				$status['run_cron'] = $status['reseller_status'];
			}
			if ($select != 'export') {
				$status_tab = $this->encode ( $table );
				$status ['table'] = "'" . $status_tab . "'";
				if ($status ['run_cron'] == 0) {
					$status_array = '<label class="switch">
					  <input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox" id=switch' . $status ['id'] . ' value=' . $status ['run_cron'] . ' onclick="javascript:processForm(' . $status ['id'] . ',' . $status ['table'] . ')" checked>
					  <span class="slider round"></span>
					</label>';
	
				} else {
					$status_array = '<label class="switch">
					  <input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox" id=switch' . $status ['id'] . ' value=' . $status ['run_cron'] . ' onclick="javascript:processForm(' . $status ['id'] . ',' . $status ['table'] . ')">
					  <span class="slider round"></span>
					</label>';
				}
			} else {
	...
	function set_search_authtype($select = '') {
		$status_array = array (
				"" => gettext ( "--Select--" ),
				"basic" => gettext ( "Basic" ),
				"password" => gettext ( "Password" ),
