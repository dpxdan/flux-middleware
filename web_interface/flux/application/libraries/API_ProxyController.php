...
abstract class API_ProxyController extends CI_Controller {
...
//		$this->early_checks ();
	...
	protected function early_checks() {
		...
		// }
		// Kinjal issue no 3405 FLUXv5 Pro Dialer API
		if (($_SERVER['HTTP_USER_AGENT'] == 'freeswitch-curl/1.0' || $_SERVER['HTTP_USER_AGENT'] == 'freeswitch-curl/1.1' || (isset($_SERVER['HTTP_X_CDR_CALLSTART']) && !empty($_SERVER['HTTP_X_CDR_CALLSTART']) && !empty($_POST) && $_SERVER['HTTP_X_CDR_CALLSTART'] == $_POST['callstart'])) && isset($_POST['uuid'])) {
			$_SERVER['HTTP_X_AUTH_TOKEN'] = $this->post('fs_key');
		}
		// END
		if($this->_post_args['action'] != 'flux_auth_token'){
			if (! isset ( $_SERVER ['HTTP_FLUX_AUTH_TOKEN'] ) || config_item ( 'api_x_auth_token' ) != $_SERVER ['HTTP_FLUX_AUTH_TOKEN']) {
				$this->response ( array (
						'status' => false,
